...
int (* const call_vec[NR_PM_CALLS])(void) = {
	...
	CALL(MB_BEROOT)			= do_mb_be_root,
	CALL(MB_ASSIGNLEADER) 	= do_mb_assign_leader,
	CALL(MB_REMOVELEADER) 	= do_mb_remove_leader,
	CALL(MB_ROOTDSEND) 		= do_mb_root_deny_send,
	CALL(MB_ROOTDRETRIEVE) 	= do_mb_root_deny_retrieve,
	CALL(MB_ROOTASEND) 		= do_mb_root_allow_send,
	CALL(MB_ROOTARETRIEVE) 	= do_mb_root_allow_retrieve,
	CALL(MB_CREATESMAILBOX) = do_mb_create_secure_mailbox,
	CALL(MB_CREATEPMAILBOX) = do_mb_create_public_mailbox,
	CALL(MB_DSEND) 			= do_mb_deny_send,
	...
	CALL(MB_EXITROOT)		= do_mb_exit_root
};
